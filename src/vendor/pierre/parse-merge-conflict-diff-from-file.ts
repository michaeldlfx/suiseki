...
// Derived from @pierre/diffs:
// https://github.com/pierrecomputer/pierre/blob/5ca34b1b809ac6f8bb13bb431e68dae1246522f4/packages/diffs/src/utils/parseMergeConflictDiffFromFile.ts
// Modifications: adjusted imports for suiseki's TypeScript layout. The function
// has zero runtime dependencies on Pierre internals; only types are imported.
//
...
export function parseMergeConflictDiffFromFile(
  ...
  const lastHunkAtAssembly = s.hunks[s.hunks.length - 1]
  if (
    lastHunkAtAssembly != null &&
    s.additionLines.length > 0 &&
    s.deletionLines.length > 0
  ) {
    const collapsedAfter = Math.max(
      s.additionLines.length -
        (lastHunkAtAssembly.additionStart +
          lastHunkAtAssembly.additionCount -
          1),
      0,
    )
