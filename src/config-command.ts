...
export function generateAnnotatedConfig(): string {
  return [
    ...
    "[view]",
    "",
    "# Show dotfiles and gitignored entries by default in `view`/`sat` (the",
    "# `--all`/`-a` behavior). `--all=false` hides them for a single invocation.",
    "# values: true | false",
    "# env:    SUISEKI_VIEW_ALL",
    `all = ${tomlValue(viewDefaults.all)}`,
    "",
