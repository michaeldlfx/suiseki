...
- [ ] **Tagline + 30-second pitch** — cover diff + file + tree.
- [ ] **Subcommand reference** — `suiseki diff`, `suiseki view`, `suiseki tree`, and `sat` (the symlink + argv0 behavior).
- [ ] **`view` / `sat` examples** — common file-viewing recipes, including `--with-tree`.
- [ ] **`tree` examples** — basic usage, git-status annotations, `--all` to override `.gitignore`, icon flag.
- [ ] **Comparison table additions** — file peers (`bat`) and tree peers (`tree`, `eza`) alongside the diff peers. Keep comparisons honest or cut them.
- [ ] **Screenshot/cast updates** — show the new subcommands.
